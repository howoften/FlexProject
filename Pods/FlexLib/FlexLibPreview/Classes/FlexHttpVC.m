...
+ (void)reloadFlexView {
    ...
    FlexHttpViewerVC *viewerVc = [self _reloadFlexView:vc];
    
    if ([viewerVc isKindOfClass:[FlexHttpViewerVC class]]) {
        [viewerVc reloadFlexView];
    }
    
}
+ (FlexHttpViewerVC *)_reloadFlexView:(UIViewController *)controller {
    if ([controller isKindOfClass:[UINavigationController class]]) {
        UIViewController *vc = [(UINavigationController *)controller topViewController];
        return [self _reloadFlexView:vc];
        
    }else if ([controller isKindOfClass:[UITabBarController class]]) {
        UIViewController *vc = [(UITabBarController *)controller selectedViewController];
        return [self _reloadFlexView:vc];
    }else if ([controller isKindOfClass:[FlexHttpViewerVC class]]) {
        return controller;
    }
    return nil;
}
